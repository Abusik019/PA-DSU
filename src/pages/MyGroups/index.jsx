        ...
        <div className="w-full h-full flex flex-col justify-start gap-[20px] items-center pt-[100px] box-border">
                    ...
                    <tbody className="bg-[#f8f3ee]">
                        {filteredGroups && filteredGroups.map(item => (
                            <tr className="p-2 box-border h-[80px]" key={item.id}>
                                ...
                                <td className="p-2 pl-4 box-border">{item && item.course}</td>
                                <td className="p-2 pl-4 box-border">{item && item.subgroup}</td>
                                <td className="p-2 box-border">{item && item.created_at.match(/\d\d\d\d-\d\d-\d\d/)}</td>
                                <td className="p-2 box-border">{item.curator && item.curator.last_name} {item.curator && item.curator.first_name.match(/[A-Za-zА-Яа-я]/)}.</td>
                                <td><Link to={`/my-groups/${item.id}`}><img src={LinkImg} width={24} height={24} alt="link img" /></Link></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
            ...
            <Modal isOpen={isFilterModal} onClose={() => setIsFilterModal(false)}>
                <div className="flex flex-col items-start gap-3 w-full h-full mt-5">
                    ...
                    <SelectDirection setFilterGroup={setFilterGroup} value={filterGroup.direction.label}/>
                    <SelectCourse setFilterGroup={setFilterGroup} value={filterGroup.course.label}/>
                    <SelectGroup setFilterGroup={setFilterGroup} value={filterGroup.group.label}/>
